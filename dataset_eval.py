import os
import torch
import numpy as np
import torchvision
import torch.utils.data
import PIL
import re
import random


class Data:
    def __init__(self, config):
        self.config = config
        self.transforms = torchvision.transforms.Compose([torchvision.transforms.ToTensor()])

    def get_loaders(self, parse_patches=True):

        train_path = os.path.join(self.config.data.train_data_dir)
        val_path = os.path.join(self.config.data.test_data_dir)

        train_dataset = MyDataset(train_path,
                                  transforms=self.transforms,
                                  n=self.config.training.patch_n,
                                  patch_size=self.config.data.image_size,
                                  parse_patches=parse_patches)
        val_dataset = MyDataset(val_path,
                                transforms=self.transforms,
                                n=self.config.training.patch_n,
                                patch_size=self.config.data.image_size,
                                parse_patches=parse_patches)

        if not parse_patches:
            self.config.training.batch_size = 1
            self.config.sampling.batch_size = 1

        # 训练数据
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=self.config.training.batch_size,
                                                   shuffle=True, num_workers=self.config.data.num_workers,
                                                   pin_memory=True)

        # 评估数据
        val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=self.config.sampling.batch_size,
                                                 shuffle=False, num_workers=self.config.data.num_workers,
                                                 pin_memory=True)

        return train_loader, val_loader


# 数据集加载类
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, dir, transforms, n, patch_size, scales=[64, 128, 256, 512, 1024], parse_patches=True):
    # def __init__(self, dir, patch_size, n, transforms, parse_patches=False):
        super().__init__()

        self.dir = dir
        self.transforms = transforms
        self.scales = scales
        self.current_scale = None
        
        input_names = os.listdir(dir+'input')
        input_names.sort()
        gt_names = os.listdir(dir+'target')
        gt_names.sort()

        self.input_names = input_names
        self.gt_names = gt_names
        self.patch_size = patch_size
        self.n = n
        self.parse_patches = parse_patches
        
    @staticmethod
    def get_params(img, output_size, n):
        w, h = img.size  # (640,480)
        th, tw = output_size # (64,64)
        if w == tw and h == th:
            return 0, 0, h, w

        i_list = [random.randint(0, h - th) for _ in range(n)]
        j_list = [random.randint(0, w - tw) for _ in range(n)]

        # 随机裁剪64*64大小的块 得到16个这个样的i,j坐标 并返回
        return i_list, j_list, th, tw

    @staticmethod
    def n_random_crops(img, x, y, h, w):
        crops = []
        for i in range(len(x)):
            new_crop = img.crop((y[i], x[i], y[i] + w, x[i] + h)) #在640*480的img种裁剪出来64*64的块
            crops.append(new_crop)

        return tuple(crops) # 根据得到16个ij坐标，得到16个这样的块

    def get_scale_for_epoch(self, epoch):
        # 计算对应的尺度索引
        index = min(epoch // 400, len(self.scales) - 1)
        return self.scales[index]
    
    def resize_image(self, image, scale):
        size = (scale, scale)
        return image.resize(size, PIL.Image.Resampling.LANCZOS)

    def set_epoch(self, epoch):
        # Update the current scale based on the new epoch
        self.current_scale = self.get_scale_for_epoch(epoch)

    # def get_images(self, index):
    #     input_name = self.input_names[index]
    #     gt_name = self.gt_names[index]
        
    #     input_img = PIL.Image.open(os.path.join(self.dir, 'input', input_name))
    #     gt_img = PIL.Image.open(os.path.join(self.dir, 'target', gt_name))

    #     # Resize images to the current scale
    #     input_img = self.resize_image(input_img, 1024)
    #     gt_img = self.resize_image(gt_img, 1024)

    #     # Apply transformations
    #     input_img = self.transforms(input_img)
    #     gt_img = self.transforms(gt_img)

    #     return torch.cat([input_img, gt_img], dim=0)




        
    def get_images(self, index):
        input_name = self.input_names[index]
        gt_name = self.gt_names[index]
        img_id = re.split('/', input_name)[-1][:-4]

        input_img = PIL.Image.open(os.path.join(self.dir, 'input', input_name)) if self.dir else PIL.Image.open(input_name)
        try:
            gt_img = PIL.Image.open(os.path.join(self.dir, 'target', gt_name)) if self.dir else PIL.Image.open(gt_name)
        except:
            gt_img = PIL.Image.open(os.path.join(self.dir, 'target', gt_name)).convert('RGB') if self.dir else \
                PIL.Image.open(gt_name).convert('RGB')

        # Resize images to the current scale
        input_img = self.resize_image(input_img, 1024)
        gt_img = self.resize_image(gt_img, 1024)
        
        if self.parse_patches:
            i, j, h, w = self.get_params(input_img, (self.patch_size, self.patch_size), self.n)
            # self.patch_size = data.image_size = (64,64) self.n = training.patch_n = 16
            # i j 每个都是包含了16个数值的list,h w都等于64
            input_img = self.n_random_crops(input_img, i, j, h, w)  # 把输入的图像裁成了16个64*64的块
            gt_img = self.n_random_crops(gt_img, i, j, h, w) # 把输入的标签裁成了16个64*64的块

            # 输出的就是16*6*64*64的torch，
            # 其中16是一张图像被裁减出来的64*64的16个块块，中间的6是拼接的3通道输入图像和3通道标签
            outputs = [torch.cat([self.transforms(input_img[i]), self.transforms(gt_img[i])], dim=0)
                       for i in range(self.n)]  # self.n = 16 来自于training.patch_n

            # torch.stack(outputs,dim=0).shape=torch.size([16,6,64,64])
            return torch.stack(outputs, dim=0), img_id
        else:
            # Resizing images to multiples of 16 for whole-image restoration
            # 将图像大小调整为16的倍数以进行全图像恢复
            wd_new, ht_new = input_img.size
            if ht_new > wd_new and ht_new > 2048:
                wd_new = int(np.ceil(wd_new * 2048 / ht_new))  # np.ceil向上取整函数
                ht_new = 2048
            elif ht_new <= wd_new and wd_new > 2048:
                ht_new = int(np.ceil(ht_new * 2048 / wd_new))
                wd_new = 2048
            wd_new = int(16 * np.ceil(wd_new / 16.0))
            ht_new = int(16 * np.ceil(ht_new / 16.0))
            input_img = input_img.resize((wd_new, ht_new), PIL.Image.ANTIALIAS)
            gt_img = gt_img.resize((wd_new, ht_new), PIL.Image.ANTIALIAS)

            return torch.cat([self.transforms(input_img), self.transforms(gt_img)], dim=0),img_id

    # 测试
    def __getitem__(self, index):
        res = self.get_images(index)
        return res

    # # 训练
    # def __getitem__(self, index):
    #     # 当 epoch >= 1200 并且 scale=512 时调用 get_images_patch，否则调用 get_images
    #     if self.current_scale >= 512:
    #         return self.get_images_patch(index)
    #     else:
    #         return self.get_images(index)

    def __len__(self):
        return len(self.input_names)
