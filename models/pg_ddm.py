...
from models.pg_unet import DiffusionPGUNet
...
# 扩散模型加噪+训练过程
class ProgressiveTraining(object):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.device = config.device
        
        self.epoch_loss_list = []  # 用于存储每个epoch的损失值

        self.model = DiffusionPGUNet(config)
    ...
    def train(self, DATASET):
        ...
        # 获取日志文件路径
        log_file = self.config.log.filename
        ...
        level=logging.INFO,
        format='%(asctime)s - Epoch: %(message)s',
        datefmt='%Y-%m-%d %H:%M:%S'
        )
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            # 根据 epoch 获取当前 scale
            current_scale = train_loader.dataset.get_scale_for_epoch(epoch)
        
            # 在每个epoch开始时更新数据集的缩放比例
            ...
            for i, x in enumerate(train_loader):
                ...
                if self.step % self.config.training.validation_freq == 0:
                    self.model.eval()
                    self.sample_validation_patches(val_loader, self.step, epoch, current_scale)

                # 保存模型
                if epoch % 50 == 0 or self.step == 1:
                # if self.step % self.config.training.snapshot_freq == 0 or self.step == 1:
                    ...
                    }, filename=self.config.training.resume + str(epoch) + '_' + str(current_scale))
                    
            self.epoch_loss_list.append((epoch, loss.item()))
            logging.info(f'{epoch}, Loss: {loss.item():.6f}')
            
        # 绘制epoch与损失值的图表
        epochs, losses = zip(*self.epoch_loss_list)
    ...
    def sample_validation_patches(self, val_loader, step, epoch, current_scale):
        image_folder = os.path.join(self.config.data.val_save_dir, str(current_scale))
        with torch.no_grad():
            ...
            if current_scale >=256:
                current_scale = self.config.data.image_size
            x = torch.randn(n, 3, current_scale, current_scale, device=self.device)
