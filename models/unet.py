...
class DiffusionUNet(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ch, out_ch, ch_mult = config.model.ch, config.model.out_ch, tuple(config.model.ch_mult) # 128 3  [1,2,3,4]转元组
        ...
        in_channels = config.model.in_channels * 2 if config.data.conditional else config.model.in_channels # 有条件就是6 没条件就是3
        resolution = config.data.image_size # 64
        ...
        self.temb.dense = nn.ModuleList([
            torch.nn.Linear(self.ch,         # 128
                            self.temb_ch),   # 512
            torch.nn.Linear(self.temb_ch,    # 512
                            self.temb_ch),   # 512
        ])
    ...
    def forward(self, x, t):
        ...
        # x[16,6,64,64]  64=64=64

        # timestep embedding
        temb = get_timestep_embedding(t, self.ch)  # 16*1的t 128  经过一些取对数和sin操作 返回16*128的结果
        temb = self.temb.dense[0](temb)   # 16*128卷成16*512返回
        temb = nonlinearity(temb)
        temb = self.temb.dense[1](temb)   # 16*512卷成16*512返回

        # 下采样
        hs = [self.conv_in(x)]  #x[16,6,64,64]变成了 [16,128,64,64]
